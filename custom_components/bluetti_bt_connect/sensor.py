...
from bluetti_bt_connect_lib import build_device, FieldName, get_unit
...
async def async_setup_entry(
    ...
    battery_net_unit = get_unit(FieldName.BATTERY_NET_POWER)
    sensors_to_add.append(
        BluettiSensor(
            coordinator,
            device_info,
            999999,
            FieldName.BATTERY_NET_POWER.value,
            unit_of_measurement=battery_net_unit,
            device_class=get_device_class(FieldName.BATTERY_NET_POWER),
            state_class=get_state_class(FieldName.BATTERY_NET_POWER),
            category=None if config.use_encryption else get_category(FieldName.BATTERY_NET_POWER),
            logger=logger,
        )
    )
