"""Bluetti BT Connect text entities."""
...
async def async_setup_entry(
    ...
    logger.info("Creating text entities for device with address %s", config.address)
    ...
    texts_to_add = []
    text_fields = bluetti_device.get_text_fields()
    ...
    async_add_entities(texts_to_add)


class BluettiText(CoordinatorEntity, TextEntity):
    """Bluetti universal text entity."""
    ...
    def __init__(
        ...
        self._attr_native_max = field.size * 2
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if not isinstance(self.coordinator.data, dict):
            self._logger.debug(
                "Invalid data from coordinator (text.%s)",
    ...
    async def write_to_device(self, value: str):
        """Write to device."""
